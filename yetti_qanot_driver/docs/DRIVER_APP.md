...
| `dart-define` | Role |
|---------------|------|
| `API_BASE_URL` | HTTPS origin, no trailing slash. Default: `https://taxi-uksz.onrender.com`. |
...
| `DRIVER_TRIP_HISTORY_HTTP_PATH` | Optional **`GET`** path override for **Safarlar tarixi** (default in app: **`/driver/trips`**). Same rules as wallet path. |
| `ENABLE_DRIVER_DISPATCH_POKE_WS` | Enable/disable the driver dispatch “poke” WebSocket (**`GET /ws/driver-dispatch`**) that sends `dispatch_changed` events; when enabled, the app immediately refetches **`GET /driver/available-requests`** on each event. Default: `true`. |
...
| Dispatch poll | `GET /driver/available-requests` | While **ONLINE**; ~3s foreground when idle for offers, ~5s when assigned/in-trip, ~45s background. **Immediate** poll when returning from background so orders placed while away show quickly. Promo-program / referral-status / referral-link run **about once per 60s** (after each dispatch ingest so queue rows are not blocked by slow promo/referral HTTP). Merges queue aliases; dedupes by `request_id`. Lifecycle-driven timer resets are **debounced** so web visibility events don’t cause poll bursts. |
