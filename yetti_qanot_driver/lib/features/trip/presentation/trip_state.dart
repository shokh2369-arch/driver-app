...
import '../domain/commission_info.dart';
...
/// Dashboard auto-offer preview window — countdown starts when the push/local notification fires.
const Duration kQueueOfferPreviewWindow = Duration(seconds: 10);
...
class TripState {
  const TripState({
    ...
    this.pendingQueueOfferExpiresAt,
    this.dispatchUnreachable = false,
    this.commission,
  });
  ...
  /// Wall-clock end of the queue-only auto-offer preview ([kQueueOfferPreviewWindow] from notify).
  final DateTime? pendingQueueOfferExpiresAt;

  /// `GET /driver/available-requests` has failed repeatedly. Surfaced on the dashboard so
  /// an unreachable backend does not just look like "no orders right now".
  final bool dispatchUnreachable;

  /// Current commission from the backend (`GET /driver/available-requests`). Null when the
  /// backend hasn't sent it (not loaded / older backend) → the UI hides the row rather than
  /// show a guessed rate. See [parseCommissionFromJson].
  ...
  List<MapLatLng> routePoints() {
    ...
    // "Route" without external Directions API: draw straight segments. Either endpoint
    // can be absent when the backend omits coordinates — never invent one.
    return [
      if (req.pickup != null) req.pickup!,
  ...
  TripState copyWith({
    ...
    DateTime? Function()? pendingQueueOfferExpiresAt,
    bool? dispatchUnreachable,
    CommissionInfo? Function()? commission,
  }) {
    return TripState(
      ...
      pendingQueueOfferExpiresAt: pendingQueueOfferExpiresAt != null
          ? pendingQueueOfferExpiresAt()
          : this.pendingQueueOfferExpiresAt,
      dispatchUnreachable: dispatchUnreachable ?? this.dispatchUnreachable,
      commission: commission != null ? commission() : this.commission,
    );
  }
}
