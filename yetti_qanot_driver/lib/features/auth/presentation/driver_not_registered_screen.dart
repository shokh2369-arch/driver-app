...
class DriverNotRegisteredScreen extends StatelessWidget {
  ...
  Future<void> _openTelegram(BuildContext context) async {
    final failureMessage = AppLocalizations.of(context).launch_link_failed;
    final uri = Uri.parse(telegramBotUrl);
    // Launch directly: `canLaunchUrl` returning false is indistinguishable from a
    // missing package-visibility entry, and this is the driver's only way to register.
    var ok = false;
    try {
      ok = await launchUrl(uri, mode: LaunchMode.externalApplication);
    } catch (e) {
      debugPrint('[yetti_driver] telegram launch failed: $e');
    }
