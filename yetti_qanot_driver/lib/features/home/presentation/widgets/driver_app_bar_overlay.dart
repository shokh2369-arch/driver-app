...
class _OnlinePillState extends State<_OnlinePill> {
  /// Going OFFLINE posts to the server and going ONLINE flushes a location fix; both
  /// take real time. Leaving the switch live let the driver flip it repeatedly and
  /// stack concurrent, conflicting status calls.
  bool _busy = false;

  Future<void> _toggle(bool nextOnline) async {
    if (_busy) return;
    setState(() => _busy = true);
    try {
      await widget.onChanged(nextOnline);
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final onlineLabel = widget.onlineLabel;
    final offlineLabel = widget.offlineLabel;
    final theme = Theme.of(context);
