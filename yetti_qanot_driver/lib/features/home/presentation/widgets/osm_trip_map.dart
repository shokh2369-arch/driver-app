...
/// Shared tile HTTP client **without** [http.RetryClient] — failed CARTO tiles must not
/// retry-storm the Network tab (same `{y}.png` dozens of times) on flaky web connections.
...
class _OsmTripMapState extends State<OsmTripMap> {
  ...
  void _followDriver(LatLng driver) {
    ...
    if (!mounted) return;
    // Avoid expensive camera moves for tiny GPS jitter.
    if (_lastFollowed != null && _km(_lastFollowed!, driver) < 0.01) {
      return;
    }
    final now = DateTime.now();
    if (_lastFollowAt != null && now.difference(_lastFollowAt!) < const Duration(milliseconds: 1500)) {
      return;
    }
    _lastFollowAt = now;
    ...
    if (bounds.contains(driver)) {
      // If still visible, skip forced recentering to keep panning smooth.
      return;
    }
    if (tripInProgress) {
  ...
  void didUpdateWidget(covariant OsmTripMap oldWidget) {
    ...
    if (req != null && oldReq != null) {
      coordsChanged = !_samePoint(req.pickup, oldReq.pickup) ||
    ...
    final markers = <Marker>[
      ...
      if (remote != null && !tripInProgress && remoteLm != null)
        Marker(
          ...
          point: remoteLm,
          child: _MapPin(color: Colors.deepPurple, icon: Icons.near_me, iconSize: 14),
        ),
          ...
          children: [
            ...
            SimpleAttributionWidget(
              ...
              onTap: () => _launchOrNotify(
                ...
                AppLocalizations.of(context).launch_maps_failed,
              ),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: Colors.white, width: iconSize >= 16 ? 2 : 1.5),
