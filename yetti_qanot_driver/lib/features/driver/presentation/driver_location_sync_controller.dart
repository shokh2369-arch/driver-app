...
/// no ISO). Timer cadence stays **under the server ~90s** live guard in foreground and when the screen is off /
/// app is backgrounded (slower tick), tighter during assigned trips.
///
...
class DriverLocationSyncController extends Notifier<int> {
  ...
  Duration _foregroundTick() {
    final tripHeavy = ref.read(tripProvider).requiresContinuousLiveLocation;
    // Keep under backend ~90s freshness guard; target ~15s while ONLINE.
    return Duration(seconds: tripHeavy ? 10 : 15);
  }

  Duration _backgroundTick() {
    final tripHeavy = ref.read(tripProvider).requiresContinuousLiveLocation;
    // Screen off / app background: stay under the backend ~90s live guard without foreground cadence.
    return Duration(seconds: tripHeavy ? 30 : 45);
  }
  ...
  Future<void> _postHttp({
    ...
    if (repo == null) return;
    try {
      // Debug-only log: verify coordinate order + accuracy + timestamp (WGS84 lat/lng).
      // Coordinates are personal data, so this must never reach release logs.
  ...
  /// Force an immediate location POST with the latest GPS fix (both native routes when applicable).
  Future<void> flushHttpNow() async {
    if (!AppConfig.hasHttpApi || !_hasAuth()) return;
    if (ref.read(driverStatusProvider) != DriverStatus.online) return;
    await _postHttp(ignoreMinInterval: true, throwOnFail: true);
  }
  ...
  void ingestPosition(Position p) {
    ...
    if (ref.read(tripProvider).hasActiveTrip) {
      if (kDebugMode && AppConfig.debugLocation) {
        debugPrint(
          '[yetti_driver] WS driver_location -> lat: ${p.latitude}, lng: ${p.longitude}',
        );
      }
